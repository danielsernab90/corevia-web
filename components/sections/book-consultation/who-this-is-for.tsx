...
export function WhoThisIsFor() {
  ...
  return (
    <Section tone="white" spacing="md" aria-labelledby="audience-heading">
      <Container size="xl">
          ...
          <StaggerGrid className="mt-10 grid w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {audienceKeys.map((key) => (
              <StaggerItem key={key} className="h-full min-w-0">
                <div className="flex h-full min-h-14 items-center justify-center rounded-xl border border-border bg-card px-4 py-5 text-center text-sm font-medium text-foreground transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:elevation-sm">
                  {t(`items.${key}`)}
                </div>
              </StaggerItem>
            ))}
